	...
	if(redata.length==285){
		for (var i = 0; i < redata.length; i += 95) {
			...
			if (parseInt(redata.substring(4 + i, 8 + i),10) == $("#machineid").val()) {
				...
				presetvol = parseFloat((parseInt(redata.substring(50 + i, 54 + i), 10) / 10).toFixed(2));
				if (symbol == 0) {
					elecurve();
					volcurve();
					symbol++;
				...
				$("#r10").html(parseInt(redata.substring(38 + i, 42 + i), 10)*parseFloat((parseInt(redata.substring(42 + i, 46 + i), 10) / 10).toFixed(2)));
