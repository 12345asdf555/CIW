...
public class ExportExcelController {
	...
	public ResponseEntity<byte[]> exporWeldingMachine(HttpServletRequest request,HttpServletResponse response){
		...
		try {
			...
			String dtime = null;
			String[] titles = new String[]{"设备编号","设备名称","设备型号","制造厂家","出厂编号","使用日期","存放地点","使用工段","上度认证时间","下次校验日期","预防性维护日期","所属项目","状态","是否在网","采集序号","ip地址"};
			...
			for(int i =0; i<list.size();i++){
				...
				data[i][3] = list.get(i).getMvaluename();
				data[i][4] = list.get(i).getFmanunumbers();
				data[i][5] = list.get(i).getJoinTime();
				data[i][6] = list.get(i).getPosition();
				data[i][7] = list.get(i).getFsection();
				data[i][8] = list.get(i).getFauthentication();
				data[i][9] = list.get(i).getFtest();
				data[i][10] = list.get(i).getFprevention();
				data[i][11] = list.get(i).getInsframeworkId().getName();
				data[i][12] = list.get(i).getStatusname();
				...
				if(gather!=null){
					data[i][14] = gather.getGatherNo();
				}else{
					data[i][14] = null;
				}
				data[i][15] = list.get(i).getIp();
			}
		...
		try{
			...
			for(DataStatistics i:list){
				...
				if(junction.getJunctionnum()!=0){
					...
					double standytimes = 0,time=0,electric=0,air=0;
					if(standytime!=null){
						standytimes = standytime.doubleValue()/60/60;
					}
					if(weldtime!=null){
						air = (double)Math.round(weldtime.getRateofflow()*100)/100;
		...
		try{
			...
			for(DataStatistics i:list){
				...
				if(junctionnum.getJunctionnum()!=0){
					...
					if(weld!=null){
						air = (double)Math.round(weld.getRateofflow()*100)/100;
